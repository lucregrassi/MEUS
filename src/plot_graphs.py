...
def show_mean_and_stddev():
    ...
    for folder in folders:
        ...
        # cvr       = [float(pd.read_csv(outpath + file)['CVR'][i]) for file in files for i in
        # range(len(pd.read_csv(outpath + file)['CVR'])) if float(pd.read_csv(outpath + file)['CVR'][i])!=-2]
...
def plot_metrics():
    ...
    for folder in folders:
        ...
        cvr = [[float(pd.read_csv(exp_path + file)['CVR'][i]) if float(
            pd.read_csv(exp_path + file)['CVR'][i]) != -2 else None for i in
                range(len(pd.read_csv(exp_path + file)['CVR']))] for file in files]
        dist = [[float(pd.read_csv(exp_path + file)['distance'][i]) for i in
                 range(len(pd.read_csv(exp_path + file)['distance']))] for file in files]
        Kalpha = [[float(pd.read_csv(exp_path + file)['Kalpha'][i]) for i in
                   range(len(pd.read_csv(exp_path + file)['Kalpha']))] for file in files]

        obss = [[list(eval(pd.read_csv(exp_path + file)['observations'][i])) for i in
                 range(len(pd.read_csv(exp_path + file)['distance']))] for file in files]
        ...
        for i in [int(file.split('.')[0]) for file in files]:
            ...
            if len(dist[count]) > 1:
                ...
                plt.plot(list(map(str, [i for i in range(len(Kalpha[count]))])), Kalpha[count], label='Kalpha',
                         marker='*')
                ...
                if len(cvr[count]) == 0:
                    ...
                    for k, el in enumerate(dist[count]):
                        ...
                        if cvr[count][k] == None:
                        ...
                        elif cvr[count][k] == 1 and len(
                                list(eval(pd.read_csv(exp_path + str(i) + '.csv')['observations'][k]))) > 1:
                            ...
                            elif index != [obs['coders'] for obs in obss1].index(
                                    np.max(obs['coders'] for obs in obss1)):
